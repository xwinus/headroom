...
# Main Features
- __License Header Management__ - [Add, replace or drop license headers][rel:running-headroom] in your source code files with a single command. Headers are recognized conservatively from their license content, so unrelated comments are preserved and no Headroom metadata is added to source files.
